                        ...
                        @foreach ($librosPendientes as $libro)
                            <li class="p-4 px-6 hover:bg-slate-50/50 flex items-center justify-between transition-colors">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-slate-800">{{ $libro->nombre }}</p>
                        ...
                        @foreach ($librosLeidos as $libro)
                            <li class="p-4 px-6 hover:bg-slate-50/50 flex items-center justify-between transition-colors">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-slate-800">{{ $libro->nombre }}</p>
                                        <p class="text-xs text-slate-400">Leído el:
                                            {{ $libro->pivot->updated_at ? $libro->pivot->updated_at->format('d/m/Y') : 'Recientemente' }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-1 bg-amber-50 border border-amber-100 px-2.5 py-1 rounded-xl">
                                    ...
                                    <span class="text-xs font-bold text-amber-800">{{ $libro->pivot->puntaje }}/5</span>
                                </div>
                            </li>
                        @endforeach
                    </ul>
